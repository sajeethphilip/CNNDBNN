...
class DatasetProcessor:
    ...
    def generate_default_config(self, train_dir, test_dir):
    ...
    def process_with_config(self):
        """Process dataset and generate configuration"""
        # Process dataset first
        train_dir, test_dir = self.process()

        # Generate default configuration
        config = self.generate_default_config(train_dir, test_dir)
...
class ConfigManager:
    ...
    def manage_all_configs(self, dataset_name: str, json_config: dict):
        ...
        # Main JSON config
        json_path = os.path.join(self.base_dir, f"{dataset_name}_config.json")
        ...
        # DBNN config
        dbnn_config_path = os.path.join(self.base_dir, f"{dataset_name}_dbnn.conf")
        ...
        # Data config
        data_config_path = os.path.join(self.base_dir, f"{dataset_name}_data.conf")
    ...
    def load_or_generate_config(self) -> dict:
        ...
        if self.check_existing_files():
            config_path = os.path.join(self.output_dir, f"{os.path.splitext(os.path.basename(self.datafile))[0]}_config.json")
            if os.path.exists(config_path):
        ...
        if not self.config['execution_flags'].get('fresh_start', False):
            ...
            if checkpoint_path:
                try:
                    ...
                    logger.info(f"Initialized model from checkpoint: {checkpoint_path}")
                except Exception as e:
...
def main(args=None):
    try:
        ...
        if args and args.config:
            ...
            # Initialize dataset processor and config manager
            processor = DatasetProcessor(datafile=datafile, datatype=datatype)
            config_manager = ConfigManager(base_dir='configs')

            # Process dataset and get initial configuration
            train_dir, test_dir, config = processor.process_with_config()
            dataset_name = config['dataset']['name']

            # Manage all configurations (main JSON, DBNN conf, and data conf)
            logger.info("Managing configurations...")
            config_manager.manage_all_configs(dataset_name, config)

            # Ask about merging datasets
        ...
        target_loss = config['training'].get('min_loss', 0.01)
        max_rounds = 1
        current_round = 0
        best_loss = float('inf')
        ...
        while current_round < max_rounds:
            current_round += 1
            logger.info(f"Starting training round {current_round}/{max_rounds}")

            # Train the model
            history = trainer.train(train_loader, test_loader)

            # Check convergence
            final_loss = min(history['train_loss']) if 'train_loss' in history else float('inf')
            best_loss = min(best_loss, final_loss)

            if final_loss <= target_loss:
                logger.info(f"Training converged! Final loss: {final_loss:.4f}")
                break
            elif current_round < max_rounds:
                logger.info(f"Training did not converge. Best loss: {best_loss:.4f}, Target loss: {target_loss:.4f}")
                retry = input("Would you like to continue training? (y/n): ").lower() == 'y'
                if retry:
                    try:
                        new_rounds = int(input("How many additional rounds? "))
                        if new_rounds > 0:
                            max_rounds = current_round + new_rounds
                            continue
                    except ValueError:
                        logger.warning("Invalid input for rounds. Stopping training.")
                break
            else:
                logger.info(f"Maximum rounds ({max_rounds}) reached without convergence. Best loss: {best_loss:.4f}")
        ...
        # Extract features for test data if available
        if test_loader:
        ...
        # Generate DBNN configuration using ConfigManager
        config_manager = ConfigManager(base_dir='configs')
        dbnn_config = config_manager.generate_dbnn_config(config)
        dbnn_config_path = os.path.join('configs', f"{dataset_name}_dbnn.conf")
        with open(dbnn_config_path, 'w') as f:
            json.dump(dbnn_config, f, indent=4)
