...
def find_dataset_pairs() -> List[Tuple[str, str, str]]:
    """
    Find all matching .conf and .csv files in current directory

    Returns:
        List of tuples (basename, conf_path, csv_path)
    """
    # Get all conf files except adaptive_dbnn.conf
    conf_files = [f for f in glob.glob("*.conf") if f != "adaptive_dbnn.conf"]
    dataset_pairs = []

    # Check each conf file for matching csv
    for conf_file in conf_files:
        ...
        if os.path.exists(csv_file):
            ...
            print(f"Found dataset pair: {conf_file} and {csv_file}")
